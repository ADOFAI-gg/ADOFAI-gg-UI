// FULL COPY FROM https://github.com/melt-ui/melt-ui/blob/f24217b93da901d8325a2784cc051715ba9628a8/src/lib/builders/hidden-input/create.ts#L18

import { createLabel } from '@melt-ui/svelte'
import { usePopper, type InteractOutsideEvent } from '@melt-ui/svelte/internal/actions'
import {
	FIRST_LAST_KEYS,
	addHighlight,
 ...
 */
export function createListbox2<
	Value,
...
>(props?: CreateListboxProps<Value, Multiple, S>) {
	...
	const activeInput = withGet(writable<HTMLElement | null>(null))
	...
	const addKeyEventListener = (node: HTMLElement, isInput: boolean) => {
		// Handle all input key events including typing, meta, and navigation.
		return addMeltEventListener(node, 'keydown', (e) => {
			const $open = open.get()
			if (!$open) return
	...
	const input = makeElement(name('input'), {
		...
		action: (node: HTMLElement): MeltActionReturn<ListboxEvents['trigger']> => {
			activeInput.set(node)
			const unusbscribe = executeCallbacks(addKeyEventListener(node, true))
